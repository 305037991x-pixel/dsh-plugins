# install-into-dsh.ps1 — 把 dsh-balance 注册进 DSH web profile 并安装链接。
#
# 幂等：重复运行安全。profile 被 DSH 桌面端/更新流程重建后（症状：余额芯片消失、
# /dsh-balance 路由 404），重跑本脚本即可恢复，然后重启 DSH web。
#
# 用法（PowerShell 7）：
#   pwsh -NoProfile -ExecutionPolicy Bypass -File "$env:USERPROFILE\.agents\skills-tools\dsh-balance\install-into-dsh.ps1"
#
...
# 1) 注册依赖 + bundle（用 node 改 JSON：保序、不破坏格式）
$register = @'
const fs = require('fs');
...
# 3) 校验链接
$link = Get-Item (Join-Path $profileDir 'node_modules\dsh-balance') -ErrorAction SilentlyContinue
if ($link -and $link.Target) {
  Write-Host "✓ 链接就绪 -> $($link.Target)"
  Write-Host '完成。重启 DSH web 后生效（重启会中断会话，页面需刷新）。'
} else {
  throw '链接未创建，请检查上方 pnpm 输出。'
}
